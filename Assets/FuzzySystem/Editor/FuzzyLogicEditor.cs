...
namespace FuzzySystem.Editor
{
    public class FuzzyLogicEditor : EditorWindow
    {
        ...
        [MenuItem("Window/FuzzySystem/New GUID")]
        private static void NewGUID()
        {
            var objs = Selection.objects;
            foreach (var obj in objs)
            {
                if (obj is TextAsset && FuzzySystem.ValidateHeader((obj as TextAsset).bytes))
                {
                    var assetPath = AssetDatabase.GetAssetPath(obj);
                    var fuzzyLogic = FuzzySystem.Deserialize((obj as TextAsset).bytes, null);
                    fuzzyLogic.guid = Guid.NewGuid().ToString();
                    var bytes = FuzzySystem.Serialize(fuzzyLogic);
        ...
        [MenuItem("Window/FuzzySystem/New GUID", true)]
        ...
        public void SetFocusedTargetGUID(string guid, float newWindowWidth, float newWindowHeight)
        {
            ...
            {
                Rect windowRect = GUIUtils.Get(FuzzySystem).editorWindow.position;
                windowRect.width = newWindowWidth;
                windowRect.height = newWindowHeight;
                GUIUtils.Get(FuzzySystem).editorWindow.position = windowRect;
            }
        }
        ...
        public FuzzySystem FuzzySystem
        {
            ...
            get
            {
                var fuzzyLogic = FuzzySystem.GetRegisteredFuzzyLogic(fuzzyLogicGUID);
        ...
        { 
            ...
            {
                FuzzySystem.UnregisterAllFuzzyLogics();
                DeleteTempShortcutsProfile();
            }
        }
            ...
            if (focusedTargetGUID == null)
            {
                GUIUtils.BeginBox();
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        bool fuzzyLogicIsChanged = GUIUtils.Get(FuzzySystem).isChanged;
                        ...
                        if (saveButton)
                        {
                            ...
                            {
                                if (i_fuzzyLogic.guid == FuzzySystem.guid)
                                {
                                    File.WriteAllBytes(filePath, FuzzySystem.Serialize(FuzzySystem));
                                    GUIUtils.Get(FuzzySystem).isChanged = false;
                                    GUIUtils.Get(FuzzySystem).editorWindow.undoStack.Empty();
                        ...
                        if (GUILayout.Button("Add Fuzzification", GUILayout.Width(120)))
                        {
                            ...
                            GUIUtils.UndoStackRecord(FuzzySystem);
                            FuzzySystem.AddFuzzification();
                        }
                        if (GUILayout.Button("Add Inference", GUILayout.Width(120)))
                        {
                            GUIUtils.GUILoseFocus();
                            GUIUtils.UndoStackRecord(FuzzySystem);
                            FuzzySystem.AddInference();
                        }

                        {
                            ...
                            FuzzySystem.evaluate = GUILayout.Toggle(FuzzySystem.evaluate, "Evaluate");
                            ...
                            GUILayout.BeginHorizontal(GUILayout.Width(150));
                            {
                                ...
                                GUIUtils.TextField(FuzzySystem, FuzzySystem.name, o => FuzzySystem.name = o);
                            }
                            ...
                            GUILayout.Label(new GUIContent(FuzzySystem.guid, "GUID"));
        ...
        private void RegisterAllFuzzyLogicsOnDisk()
        {
            ...
            {
                // Register it
                if (FuzzySystem.IsRegisteredFuzzyLogic(fuzzyLogic.guid) == false)
                {
                    FuzzySystem.RegisterFuzzyLogic(fuzzyLogic);
        ...
        private void ForEachFuzzyLogicsOnDisk(Func<FuzzySystem, string/*filePath*/, bool/*go on iterating*/> action)
        {
            ...
            foreach (var file in allFiles)
            {
                ...
                if (FuzzySystem.ValidateHeader(bytes))
                {
                    FuzzySystem fuzzySystem = FuzzySystem.Deserialize(bytes, null);
                    if (action != null)
                    {
                        if (action(fuzzySystem, file) == false)
                        {
                            break;
                        }
                    }
        ...
        private void OnGUI_RefreshAllFuzzyLogics()
        {
            if(GUILayout.Button(new GUIContent(string.Empty, "Reload all FuzzyLogics from disk.\nChanges not saved will be lost.\nIt's useful when you modify(rename, delete or move etc.) saved data in folders."), refreshButtonStyle, GUILayout.Width(18), GUILayout.Height(18)))
            {
                ...
                bool doSave = true;
                if (GUIUtils.Get(FuzzySystem).isChanged)
                {
                ...
                if (doSave)
                {
                    for (int i = 0; i < FuzzySystem.NumberRegisteredFuzzyLogics(); i++)
                    {
                        var fuzzyLogic = FuzzySystem.GetRegisteredFuzzyLogic(i);
                    ...
                    GUIUtils.Get(FuzzySystem).ShowNotification("Reload Complete");
                }
            }
            ...
            if (selectedIndex != newSelectedIndex)
            {
                ...
                if (GUIUtils.Get(FuzzySystem).isChanged)
                {
                    if(EditorUtility.DisplayDialog("Message", "Some changes are not saved, would you still like to close it?\nChanges not saved will be lost.", "Yes", "Cancel"))
                    {
                        ForEachFuzzyLogicsOnDisk((i_fuzzyLogic, filePath)=>
                        {
                            if (FuzzySystem.guid == i_fuzzyLogic.guid)
                            { 
                                GUIUtils.Get(FuzzySystem).isChanged = false;
                                GUIUtils.Get(FuzzySystem).editorWindow.undoStack.Empty();
                                FuzzySystem.Deserialize(FuzzySystem.Serialize(i_fuzzyLogic), FuzzySystem);
                                return false;
                            }
                            else
                            {
